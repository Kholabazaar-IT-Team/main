...
// send firebase notfications
if(! function_exists('send_notification_FCM')){
    function send_notification_FCM($title, $message, $id="",$type,$image_url="",$click_action="") {
    ...
    function get_cached_products($category_id = null) {
        ...
        if ($category_id != null) {
            return Cache::remember('products-category-'.$category_id, 86400, function () use ($category_id, $products) {
                ...
                // ->where('featured','1')
                return $products->whereIn('category_id', $category_ids)->where('featured','1')->latest()->take(12)->with('stocks')->get();
            });
        }
        else {
            return Cache::remember('products', 86400, function () use ($products) {
                return $products->latest()->with('stocks')->get();
            });
        }
    }
}
