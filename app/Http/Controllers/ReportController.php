...

{
    ...
    public function user_search_report(Request $request){
        ...
        if($request->has('qury') && $request->query!=''){
            $q=$request->qury;
            $searches = $searches->where('qury','like','%'.$q.'%');
        }
        $searches = $searches->orderBy('count', 'desc')->paginate(10);
        return view('backend.reports.user_search_report', compact('searches','q'));
    }
