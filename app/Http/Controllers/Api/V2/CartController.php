    ...
    public function summary($user_id, $owner_id)
    {
        ...
        if ($items->isEmpty()) {
            return response()->json([
                'sub_total' => (double) 0.00,
                'tax' => (double) 0.00,
                'shipping_cost' => (double) 0.00,
                'discount' => (double) 0.00,
                'grand_total' => (double) 0.00,
                'grand_total_value' => (double) 0.00,
                'coupon_code' => "",
        ...
        foreach ($items as $cartItem) {
            $item_sum = 0;
            $product =  Product::where('id', $cartItem["product_id"])->where('approved', '1')->where('published', '1')->where(function($p) use ($verified_sellers){
                $p->where('added_by', 'admin')->orWhere(function($q) use ($verified_sellers){
                    $q->whereIn('user_id', $verified_sellers);
                });
            })->first();
          if($product){
            $p_stock=$product->stocks->where('variant', $cartItem['variation'])->first();
            if(intval($p_stock->qty) >= intval($cartItem["quantity"])){
                $item_sum += ( formated_discounted_price($product,$p_stock) + $cartItem->tax) * $cartItem->quantity;
        ...
        return response()->json([
            'sub_total' => (double) $items->sum('price'),
            'tax' => (double) $items->sum('tax'),
            'shipping_cost' => (double) $items->sum('shipping_cost'),
            'discount' => (double) $items->sum('discount'),
            'grand_total' => (double) $sum,
            'grand_total_value' => (double) convert_price($sum),
        ...
        $shops = [];
                ...
                if (!empty($shop_items_raw_data)) {
                    ...
                    foreach ($shop_items_raw_data as $shop_items_raw_data_item) {
                        $product =  Product::where('id', $shop_items_raw_data_item["product_id"])->where('approved', '1')->where('published', '1')->where(function($p) use ($verified_sellers){
                                $p->where('added_by', 'admin')->orWhere(function($q) use ($verified_sellers){
                                    $q->whereIn('user_id', $verified_sellers);
                                });
                            })->first();
                       if($product){
                           $p_stock=$product->stocks->where('variant', $shop_items_raw_data_item['variation'])->first();
                           if(intval($p_stock->qty) >= intval($shop_items_raw_data_item["quantity"])){
                                ...
                                $shop_items_data_item["price"] =(double) formated_discounted_price($product,$p_stock);
                                ...
                                $shop_items_data_item["upper_limit"] = intval($p_stock->qty);

                                $shop_items_data[] = $shop_items_data_item;
                           }else{
                            Cart::destroy( $shop_items_data_item["id"]);
                           }
                       }else{
                            Cart::destroy( $shop_items_data_item["id"]);
                       }
