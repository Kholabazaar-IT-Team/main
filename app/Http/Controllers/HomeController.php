...
class HomeController extends Controller
{
    ...
    {
        // dd($request);
        $user = User::whereIn('user_type', ['customer', 'seller'])->where('email', $request->email)->orWhere('phone','+88'.$request->email)->first();
        if($user != null){
    ...
    public function flash_deal_details(Request $request, $slug,$category_id=null)
    {
        $flash_deal = FlashDeal::where('slug', $slug)->first();
        if($flash_deal != null){
            if($category_id!=null){
                ...
                $products=FlashDealProduct::where('flash_deal_id',$flash_deal->id)->paginate(15);
                $html='';
                //print_r($slug);exit();
                if($request->ajax()){
        ...
        $html.='<div class="col mb-2" ><div class="position-relative p-1">';
        if($discount_percent > 0){
       $html.='<span class="badge badge-inline badge-danger absolute-top-right z-1 fs-12 text-uppercase px-2 py-1 d-inline-block h-auto" style="background:#f00">'.$discount_percent.' % OFF</span>';
     }
       $html.='<a href="'.route('product', $product->slug).'" class="d-block"><img class="img-fit lazyload mx-auto" src="'.static_asset('assets/img/placeholder.jpg').'"
               data-src="'.uploaded_asset($product->thumbnail_img).'"
               alt="'.$product->getTranslation('name').'"
               class="" onerror="this.onerror=null;this.src="'.static_asset('assets/img/placeholder.jpg').'"></a></div><div class="px-md-3 px-2 pt-3 text-center pb-0">
               ...
               <div class="fs-13 fw-700 pb-1" style="height:40px;">';
                   ...
                   if(home_base_price($product) != home_discounted_base_price($product)){
                    $html.='<del class=" opacity-60 mr-1">'.home_base_price($product).'</del>';
    }
    $html.='<span class="fs-14 fw-700 pb-1">'.home_discounted_base_price($product).'</span>
               </div>
           </div>
           <div class="d-flex justify-content-center align-items-center border-top border-gray-100">';
          if($stock->qty < 1)
       
          $html.='<button type="button" class="btn text-danger fw-600 fs-13" disable>'.translate('STOCK OUT').'</button>';
       else{
           if($product->variant_product == 1){
       
            $html.='<button type="button" class="btn text-alter fw-600 fs-13" onclick="showAddToCartModal('.$product->id.')">'.translate('Add To Cart').'</button>';
       
           }else{
       
            $html.='<button type="button" class="btn text-alter fw-600 fs-13" onclick="addToCart(this)" data-id="'.$product->id.'">'.translate('Add To Cart').'</button>';
...
//        $products = filter_products(Product::where('published', 1)->where('name', 'like', '%'.$request->search.'%'))->orWhere('tags', 'like', '%'.$request->search.'%')->get()->take(3);
 ...
 public function todaydeals(Request $request){
        return $this->search($request, null,null,true);
    ...
    public function search(Request $request, $category_id = null, $brand_id = null)
    {
        ...
        $products = Product::OrderBy('featured', 'desc')->where($conditions);
        ...
        $products = filter_products($products)->paginate(get_setting('per_page_pagination'))->appends(request()->query());
...
//        }
    ...
    public function reset_password_with_code(Request $request){
        if (($user = User::where('email', $request->email)->orWhere('phone','like','%'.$request->email.'%')->where('verification_code', $request->code)->first()) != null) {
    ...
    public function all_seller(Request $request) {
        ...
        if($request->q && $request->q!=''){
            $shops=$shops->where('name','like','%'.$request->q.'%');
            $q=$request->q;
        }
        $shops=$shops->paginate(15);
        return view('frontend.shop_listing', compact('shops','q'));
    }
